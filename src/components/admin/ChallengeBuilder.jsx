...
export default function ChallengeBuilder() {
  ...
  const handleSave = async () => {
    ...
    if (editingChallenge === "new") {
      ...
      await supabase.from('group_challenges').insert(payload);
    } else {
      await supabase.from('group_challenges').update(payload).eq('id', editingChallenge.id);
    }
  ...
  const handleDelete = async (id) => {
    await supabase.from('group_challenges').delete().eq('id', id);
    queryClient.invalidateQueries({ queryKey: ["groupChallenges"] });
  };
                ...
                {chs.map((ch) => (
                  <div key={ch.id} className="px-4 py-3 flex items-start justify-between gap-3">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <button onClick={() => openEdit(ch)} className="p-1.5 rounded-lg hover:bg-secondary text-muted-foreground hover:text-foreground transition-colors">
                        <Pencil className="w-3.5 h-3.5" />
                      </button>
                      <button onClick={() => handleDelete(ch.id)} className="p-1.5 rounded-lg hover:bg-destructive/10 text-muted-foreground hover:text-destructive transition-colors">
        ...
        <DialogContent className="bg-card border-border max-w-md max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 pt-1">
            <div>
              ...
              <Input value={form.title} onChange={(e) => setForm((p) => ({ ...p, title: e.target.value }))} className="bg-secondary border-0 mt-1.5" placeholder="Short challenge name" />
            </div>
            <div>
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Description</label>
              <Textarea value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} className="bg-secondary border-0 mt-1.5 resize-none" rows={3} placeholder="What does the disciple need to do?" />
            </div>
            <div>
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Expected Outcome</label>
              <Textarea value={form.outcome} onChange={(e) => setForm((p) => ({ ...p, outcome: e.target.value }))} className="bg-secondary border-0 mt-1.5 resize-none" rows={2} placeholder="What growth or result is expected?" />
            </div>
            <div>
              ...
              {form.category === "__new__" && (
                <Input value={newCategory} onChange={(e) => setNewCategory(e.target.value)} className="bg-secondary border-0 mt-2" placeholder="New category name" />
              )}
            </div>
            <div>
            ...
            <Button onClick={handleSave} disabled={saving || !form.title.trim() || !form.category.trim()} className="w-full bg-primary text-primary-foreground font-semibold">
