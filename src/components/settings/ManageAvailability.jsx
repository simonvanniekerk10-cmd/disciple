  ...
  return (
    <div className="bg-card rounded-2xl border border-border p-5 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" onClick={() => setShowAdd(true)} className="bg-primary text-primary-foreground font-semibold rounded-full h-8 px-3">
      ...
      {upcoming.length === 0 ? (
        ...
        <div className="space-y-3">
          {upcoming.map((slot) => (
            <div key={slot.id} className={`rounded-xl p-4 border ${slot.status === "booked" ? "bg-primary/5 border-primary/20" : "bg-secondary border-transparent"}`}>
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-start gap-3">
                  <div className="w-8 h-8 rounded-full bg-card flex items-center justify-center shrink-0 mt-0.5">
                    {slot.type === "phone" ? <Phone className="w-3.5 h-3.5 text-primary" /> : <MapPin className="w-3.5 h-3.5 text-primary" />}
                  </div>
                  <div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  {slot.status === "booked" ? (
                    <Button variant="ghost" size="icon" onClick={() => handleCancelBooking(slot)} className="text-muted-foreground hover:text-destructive h-7 w-7">
                      <X className="w-3.5 h-3.5" />
                    </Button>
                  ) : (
                    <Button variant="ghost" size="icon" onClick={() => handleDelete(slot)} className="text-muted-foreground hover:text-destructive h-7 w-7">
        ...
        <DialogContent className="bg-card border-border max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input type="date" value={slotDate} onChange={(e) => setSlotDate(e.target.value)} className="bg-secondary border-0 mt-1" />
            </div>
            <div>
              <Label className="text-xs text-muted-foreground uppercase tracking-wider">Time</Label>
              <Input type="time" value={slotTime} onChange={(e) => setSlotTime(e.target.value)} className="bg-secondary border-0 mt-1" />
            </div>
            <div>
            ...
            <Button onClick={handleAdd} disabled={saving || !slotDate || !slotTime} className="w-full bg-primary text-primary-foreground font-semibold">
