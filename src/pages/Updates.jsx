...
export default function Updates() {
  ...
  const [editingUpdate, setEditingUpdate] = useState(null); // null = new, or update object
  ...
  const { data: groupSettings } = useQuery({
    ...
    queryFn: () =>
      base44.entities.GroupSettings.filter({ oversight_leader_id }, "-created_date", 1),
  ...
  const handleSubmit = async () => {
    ...
    if (editingUpdate) {
      ...
      if (activeChallenge.status === "not_started") {
        await base44.entities.ChallengeSelection.update(activeChallenge.id, { status: "in_progress" });
      }
    }
  ...
  const handleComplete = async () => {
    if (!activeChallenge) return;
    setCompleting(true);
    await base44.entities.ChallengeSelection.update(activeChallenge.id, {
      status: "completed",
